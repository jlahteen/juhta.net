    ...
    public class CommandLineParser
    {
        ...
        public void ParseArguments(string[] arguments, string argumentNamePrefix, string optionPrefix, string optionNameValueSeparator)
        {
            ...
            if (m_argumentNamePrefix == m_optionPrefix)
                throw new CommandLineParserException(LibraryMessages.Error005.GetMessage());
        ...
        private OptionArgument CreateOptionArgument(string optionArgument)
        {
            ...
            else
            {
                ...
                if (String.IsNullOrWhiteSpace(optionName))
                    throw new CommandLineArgumentException(LibraryMessages.Error004.FormatMessage(fullOptionArgument));

                optionValue = optionArgument.Substring(nameValueSeparatorPosition + m_optionNameValueSeparator.Length);

                if (String.IsNullOrWhiteSpace(optionValue))
                    throw new CommandLineArgumentException(LibraryMessages.Error006.FormatMessage(fullOptionArgument));
            }

            if (!Regex.IsMatch(optionName, c_regexOptionName))
                throw new CommandLineArgumentException(LibraryMessages.Error007.FormatMessage(fullOptionArgument, c_regexOptionName));
        ...
        private string RemovePossiblePrefix(ref string argument)
        {
            ...
            else if (argument.StartsWith(m_optionPrefix))
            {
                ...
                if (String.IsNullOrWhiteSpace(argument))
                    throw new CommandLineArgumentException(LibraryMessages.Error003.FormatMessage(m_optionPrefix));
