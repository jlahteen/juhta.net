    ...
    public class XmlDocumentValidator : IValidator<XmlDocument>
    {
        ...
        public void Validate(XmlDocument document)
        {
            if (!m_schemaSet.Contains(document.DocumentElement.NamespaceURI))
                throw new XmlSchemaValidationException(LibraryMessages.Error001.FormatMessage(document.DocumentElement.NamespaceURI));
            ...
            if (m_validationException != null)
                throw new ValidationException(LibraryMessages.Error004.GetMessage(), m_validationException);
        }
